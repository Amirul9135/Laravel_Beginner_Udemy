...
export default class Search {
  ...
  async sendRequest() {
    const results = await axios(`/api/posts/find/${this.inputField.value}`)
    ...
    if (posts.length) {
      ...
      ${posts
          .map(post => {
            ...
            return `<a href="/api/posts/${post.id}" class="list-group-item list-group-item-action">
        <img class="avatar-tiny" src="${post.posted_by.avatar}"> <strong>${post.title}</strong>
        <span class="text-muted small">by ${post.posted_by.username} on ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
      </a>`
          })
          .join("")}
    </div>`)
